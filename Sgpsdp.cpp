...
// returns a string containing the version of this dll.
DWORD WINAPI AGSat_GetVersion(CHAR szVersion[])
{
	strcpy(szVersion, "Satellite Tracking DLL V1.00 (c) 2000 Alex Greenland\0");
...
VECTOR	DLLEXPORT GetUserPos(void)	{return m_vUPos;}
VECTOR	DLLEXPORT GetUserVel(void)	{return m_vUVel;}
VECTOR	DLLEXPORT GetObserver(void)	{return m_vObs;}
VECTOR	DLLEXPORT GetRADec(void)		{return m_vRad;}
...
BOOL WINAPI AGSat_ThreeLineSetup(CHAR cLine0[], CHAR cLine1[], CHAR cLine2[])
{
	...
	Init();
	//AGSat_AutoSGPSDP(0);
	//CalculateLatLonAlt(0);
	return true;
}

//void CSGP4_SDP4::SetSatellite (char *cLine0, char *cLine1, char *cLine2)
void AGSat_SetSatelliteThreeLine (char *cLine0, char *cLine1, char *cLine2)
{
	memcpy(m_cLine0, cLine0, 22);
	memcpy(m_cLine1, cLine1, 69);
	memcpy(m_cLine2, cLine2, 69);
...
void DLLEXPORT AGSat_SatelliteSetup(SATELLITE *pSatellite)
{
	SetSatellite(pSatellite);
	return;
}

//void CSGP4_SDP4::SetSatellite(SATELLITE *pSatellite)
...
// Now everything is in butter !
	...
	m_Sat.fJulianEpoch = JulianDateFromEpochTime (iEpochYear*1000.0+m_Sat.iEpochDay+m_Sat.fEpochFraction);
	Init();
	return;
}
...
VECTOR DLLEXPORT GetPos()	{return m_vPOS;}
VECTOR DLLEXPORT GetVel() {return m_vVEL;}
double DLLEXPORT GetTime(){return m_fTime;}

//double CSGP4_SDP4::GetLat() 
double DLLEXPORT GetLat() 
{
...
//	m_Sat.fRadiationCoefficient = GetFloat	(54,61,m_cLine1)*1e-5;	// = bstar 54, 6 lang
	...
	m_Sat.fJulianEpoch = JulianDateFromEpochTime (m_Sat.iEpochYear *1000.0+ m_Sat.fEpochDay);
...
//double CSGP4_SDP4::AcTan(double sinx, double cosx)
...
double DLLEXPORT JulianDateFromSystemTime (SYSTEMTIME st)
{
...
double DLLEXPORT JulianDateFromCTime (CTime ct)
{
    int days[] = {31,28,31,30,31,30,31,31,30,31,30,31};
	int i;
	double arg, arg2, fFraction;
	int iYear = (int)ct.GetYear()-1;
	int iDay  = (int)ct.GetDay();
	double A,B;

	for (i=0;i < ct.GetMonth()-1;i++) iDay += days[i];	// addiere die Tage des Jahres ...
	if	(((ct.GetYear() %4) == 0) &&
		(((ct.GetYear() %100) != 0) || ((ct.GetYear() %400) == 0)) &&
		(ct.GetMonth() > 2)) iDay += 1;		// Schaltjahr ???
...
//	modf(30.6001 * 14, &arg2);
	arg2 = 428.0;
	arg += arg2 + 1720994.5 + B;
	fFraction = (double) (ct.GetHour()*60.0*60.0	+ 
						  ct.GetMinute()*60.0		+
						  ct.GetSecond()         )/(24.0*60.0*60.0);
//						  ct.wMilliseconds/100)/(24*60*60);Not precise enough for milliseconds
	return arg+iDay+fFraction;	// The JulianDate of the CTime Date
}

//double CSGP4_SDP4::JulianDate (double st)
double DLLEXPORT JulianDateFromEpochTime (double st)
{
	double dYear;
...
SYSTEMTIME DLLEXPORT CalendarDate (double dJulian)
{
...
double DLLEXPORT SideralTime(double jd)
{
...
//void CSGP4_SDP4::CalculateLatLonAlt(double time)	//,VECTOR pos)
void DLLEXPORT CalculateLatLonAlt(double time)	//,VECTOR pos)
{
...
BOOL WINAPI AGSat_AutoSGPSDP(double time)
{
	...
	if (ideep == 0)
		ret = AGSat_SGP4(tsince,&iflag,&pos,&vel);	// Calculation for near earth
	else
		ret = AGSat_SDP4(tsince,&iflag,&pos,&vel);	// Calculation for the deep space
